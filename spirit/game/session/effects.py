...
class EffectContext:
    ...
    async def apply_special_condition(
        ...
        if conditions_blocked(self.board, target, condition):
            ...
            self._queue_effect_prevented(target)
            return False
        # Register affected targets as visual targets to prevent incorrect 
        # Fizzled animation from playing.
        if target.entity_id not in self.visual_targets:
            self.visual_targets.append(target.entity_id)
    ...
    async def assemble_legend(self, first: LegendHalfEntity, second: LegendHalfEntity):
        """Play a compatible pair from your hand, including its on-play abilities."""
...
def is_pokemon_card(card: CardEntity) -> bool:
    return card.get_attribute(AttrID.CARD_TYPE) in (CardType.POKEMON.value, CardType.LEGEND_HALF.value)
