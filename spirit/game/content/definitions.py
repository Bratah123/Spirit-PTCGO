...
class DeckDef(ProductDef):
    """A fixed 60-card product; ThemeDeckDef additionally approves Theme play."""

    def __init__(self, *, contents, description="", image_name=None, **kwargs):
        kwargs['product_type'] = ProductType.DECKS
        if image_name is not None:
            if kwargs.get("image_url") is not None:
                raise ValueError("Use image_name or image_url, not both")
            kwargs["image_url"] = image_name
        super().__init__(**kwargs)
        self.guid = str(uuid.UUID(self.guid))
        self.contents = tuple(contents)
        if not all(isinstance(entry, DeckEntry) for entry in self.contents):
            raise ValueError("contents must contain DeckEntry objects")
        self.description = description
        self.theme_legal = False
        self.is_theme_deck = False

    def resolve_contents(self, card_registry):
        resolver = card_registry if isinstance(card_registry, CardReferenceResolver) \
            else CardReferenceResolver(card_registry)
        return [guid for entry in self.contents
                for guid in [resolver.resolve(entry.card)] * entry.count]

    def validate_definition(self, card_registry):
        resolver = card_registry if isinstance(card_registry, CardReferenceResolver) \
            else CardReferenceResolver(card_registry)
        try:
            contents = self.resolve_contents(resolver)
        except ValueError as exc:
            return [str(exc)]
        validator = DeckValidator({"piles": {"deck": contents}}, card_registry=resolver)
        return [detail["explanation"]["id"] for detail in validator.base_failures()]

    def to_archetype_dict(self):
        archetype = super().to_archetype_dict()
        attrs = archetype["attributes"]
        attrs[str(AttrID.PRODUCT_TYPE.value)] = {"type": "int", "value": ProductType.DECKS.value}
        attrs[str(AttrID.IS_THEME_DECK.value)] = {"type": "bool", "value": self.is_theme_deck}
        attrs[str(AttrID.DECK_CONTENTS_ID.value)] = {"type": "uuid", "value": self.guid}
        if self.description:
            attrs[str(AttrID.DESCRIPTION.value)] = {
                "type": "json", "value": json.dumps({"id": self.description})}
        return archetype


class ThemeDeckDef(DeckDef):
    """A deck product whose exact card list can be approved for Theme format."""

    def __init__(self, *, theme_legal=True, **kwargs):
        if type(theme_legal) is not bool:
            raise ValueError("theme_legal must be a boolean")
        super().__init__(**kwargs)
        self.is_theme_deck = True
